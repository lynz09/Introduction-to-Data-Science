...
def pairwise_item_sim(R):
    ...
    for i in range(m):
        for j in range(m):
            S_matrix = np.vstack([R[i], R[j]])
            S_matrix = S_matrix[:, ~np.any(S_matrix == None, axis=0)]
            S[i, j] = cosine_similarity(S_matrix[0], S_matrix[1])
...
def predict(R,S,i,j):
    ...
    w = 0
    for k in range(m):
        if k != i and R[k, j] != None:
            score = R[k, j]*S[k, i]
            #print(R[k, j],S[k, i],score)
            weight = S[k, i]
            #print(weight)
            sum += score
            w += weight
