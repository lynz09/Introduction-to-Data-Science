...
def sum_salaries(T, D):
    ...
    S = sum_column(D[D['playerID'].isin(T)], key='salary')
...
def sum_stat(T, D, key='H'):
    ...
    #########################################
    ## INSERT YOUR CODE HERE
    S = sum_column(D[D['playerID'].isin(T)], key=key)
...
def runs(T, D):
    ...
    #########################################
    ## INSERT YOUR CODE HERE
    total_H = sum_stat(T, D, key='H')
    total_2B = sum_stat(T, D, key='2B')
    total_3B = sum_stat(T, D, key='3B')
    total_HR = sum_stat(T, D, key='HR')
    total_BB = sum_stat(T, D, key='BB')
    total_AB = sum_stat(T, D, key='AB')
    RC = runs_created(total_H, total_2B, total_3B, total_HR, total_BB, total_AB)
    #########################################
    return RC 
...
def scout():
    ...
    #########################################
    ## INSERT YOUR CODE HERE
    T = ['hattesc01', 'foulkke01', 'giambje01']
...
def rank_BA(D, t=300,max_salary=1200000):
    ...
    ## INSERT YOUR CODE HERE
    D['BA'] = D['H']/D['AB']
    D.loc[(D['AB'] < t) | (D['salary'] > max_salary), ['BA']] = 0
    R = list(D.sort_values(by=['BA'], ascending=False)['playerID'])
...
def rank_OBP(D,t=300,max_salary=1200000):
    ...
    D.loc[(D['AB'] < t) | (D['salary'] > max_salary), ['OBP']] = 0
    R = list(D.sort_values(by=['OBP'], ascending=False)['playerID'])
